...
(async () => {
  const accounts = await client.database.getAccounts(snapshotSecrets.map(s => s.hiveAccount));
  ...
  const operations = accounts.map(a => {
    ...
    const encryptedMemo = steem.memo.encode(process.env.MEMO_KEY_HIVE, a.memo_key, memo);
    return ["transfer", {
  ...
  const response = await client.broadcast.sendOperations( operations , PrivateKey.fromString(process.env.ACTIVE_KEY_HIVE) );
