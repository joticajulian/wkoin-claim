/******/ (() => {
  ...
  /******/ var __webpack_modules__ = {
    ...
    /***/ 4421: /***/ (__unused_webpack_module, exports) => {
      "use strict";
    ...
    /***/ 9822: /***/ (
      ...
      class Contract {
        constructor(c) {
          var _a;
          if (c.id) this.id = (0, utils_1.decodeBase58)(c.id);
              ...
              this.functions[name] = async (argu = {}, options) => {
                ...
                if (readOnly) {
                  ...
                  const { result: resultEncoded } =
                    await this.provider.readContract(operation.call_contract);
                ...
                const { transaction, receipt } =
                  await this.signer.sendTransaction(tx, optsSend);
        ...
        async encodeOperation(op) {
          ...
          if (method.argument) {
            if (!op.args)
              throw new Error(
                `No arguments defined for type '${method.argument}'`
              );
        ...
        async decodeOperation(op) {
          ...
          if (
            op.call_contract.contract_id !== (0, utils_1.encodeBase58)(this.id)
          )
            throw new Error(
              `Invalid contract id. Expected: ${(0, utils_1.encodeBase58)(
                this.id
              )}. Received: ${op.call_contract.contract_id}`
            );
    ...
    /***/ 5635: /***/ (
      ...
      class Provider {
        ...
        async getNonce(account, deserialize = true) {
          const { nonce: nonceBase64url } = await this.call(
          ...
          const message =
            protocol_proto_js_1.koinos.chain.value_type.decode(valueBuffer);
          const object = protocol_proto_js_1.koinos.chain.value_type.toObject(
            message,
            {
              longs: String,
              defaults: true,
         ...
         */
        async getChainId() {
          const { chain_id: chainId } = await this.call(
            "chain.get_chain_id",
            {}
          );
          return chainId;
        ...
        async sendTransaction(transaction, broadcast = true) {
          const response = await this.call("chain.submit_transaction", {
            transaction,
            broadcast,
          });
      ...
      class Serializer {
        ...
        btypeDecode(valueBtypeEncoded, protobufType, verifyChecksum) {
          ...
          Object.keys(protobufType.fields).forEach((fieldName) => {
            // @ts-ignore
            const { options, name, type, rule } =
              protobufType.fields[fieldName];
            if (!valueBtypeEncoded[name]) return;
            const typeField = { type };
            if (options) {
              if (options[OP_BYTES_1]) typeField.btype = options[OP_BYTES_1];
              else if (options[OP_BYTES_2])
                typeField.btype = options[OP_BYTES_2];
            }
            // arrays
            if (rule === "repeated") {
              valueBtypeDecoded[name] = valueBtypeEncoded[name].map(
                (itemEncoded) => {
                  // custom objects
                  ...
                  // native types
                  return (0, utils_1.btypeDecodeValue)(
                    itemEncoded,
                    typeField,
                    verifyChecksum
                  );
            ...
            // native types
            valueBtypeDecoded[name] = (0, utils_1.btypeDecodeValue)(
              valueBtypeEncoded[name],
              typeField,
              verifyChecksum
            );
        ...
        btypeEncode(valueBtypeDecoded, protobufType, verifyChecksum) {
          ...
          Object.keys(protobufType.fields).forEach((fieldName) => {
            ...
            // arrays
            if (rule === "repeated") {
              valueBtypeEncoded[name] = valueBtypeDecoded[name].map(
                (itemDecoded) => {
                  // custom objects
                  if (!nativeTypes.includes(type)) {
                    const protoBuf = this.root.lookupTypeOrEnum(type);
                  ...
                  // native types
                  return (0, utils_1.btypeEncodeValue)(
                    itemDecoded,
                    typeField,
                    verifyChecksum
                  );
            ...
            // native types
            valueBtypeEncoded[name] = (0, utils_1.btypeEncodeValue)(
              valueBtypeDecoded[name],
              typeField,
              verifyChecksum
            );
      ...
      class Signer {
        ...
        getPrivateKey(format = "hex", compressed = false) {
          ...
          switch (format) {
            ...
            case "wif":
              return (0, utils_1.bitcoinEncode)(
                (0, utils_1.toUint8Array)(stringPrivateKey),
                "private",
                comp
              );
            default:
        ...
        async sendTransaction(transaction, options) {
          ...
          if (
            ...
            !((_a = transaction.signatures) === null || _a === void 0
              ? void 0
              : _a.length)
          )
        ...
        static recoverAddress(hash, signature, compressed = true) {
          return (0, utils_1.bitcoinAddress)(
            (0, utils_1.toUint8Array)(
              Signer.recoverPublicKey(hash, signature, compressed)
            )
          );
        }
        /**
        ...
        async recoverPublicKeys(txOrBlock, opts) {
          ...
          if (block.signature) {
            ...
            const headerDecoded = (0, utils_1.btypeDecode)(
              block.header,
              btypeBlockHeader,
              false
            );
            const message =
              protocol_proto_js_1.koinos.protocol.block_header.create(
                headerDecoded
              );
            headerBytes = protocol_proto_js_1.koinos.protocol.block_header
              .encode(message)
              .finish();
          } else {
            ...
            const message =
              protocol_proto_js_1.koinos.protocol.transaction_header.create(
                headerDecoded
              );
            headerBytes = protocol_proto_js_1.koinos.protocol.transaction_header
              .encode(message)
              .finish();
          }
          ...
          return Promise.all(
            signatures.map(async (signature) => {
              ...
              return Signer.recoverPublicKey(
                hash,
                (0, utils_1.decodeBase64url)(signature),
                compressed
              );
            })
          );
        ...
        async recoverAddresses(txOrBlock, opts) {
          ...
          return publicKeys.map((publicKey) =>
            (0, utils_1.bitcoinAddress)((0, utils_1.toUint8Array)(publicKey))
          );
        }
        /**
        ...
        async prepareTransaction(tx) {
          ...
          const payer =
            (_a = tx.header.payer) !== null && _a !== void 0
              ? _a
              : this.address;
          ...
          if (tx.operations) {
            for (
              let index = 0;
              index <
              ((_b = tx.operations) === null || _b === void 0
                ? void 0
                : _b.length);
              index += 1
            ) {
              const operationDecoded = (0, utils_1.btypeDecode)(
                tx.operations[index],
                btypesOperation,
                false
              );
              const message =
                protocol_proto_js_1.koinos.protocol.operation.create(
                  operationDecoded
                );
              const operationEncoded =
                protocol_proto_js_1.koinos.protocol.operation
                  .encode(message)
                  .finish();
              operationsHashes.push((0, sha256_1.sha256)(operationEncoded));
            }
          }
          const operationMerkleRoot = (0, utils_1.encodeBase64url)(
            new Uint8Array([
          ...
          const headerBytes =
            protocol_proto_js_1.koinos.protocol.transaction_header
              .encode(message)
              .finish();
          const hash = (0, sha256_1.sha256)(headerBytes);
          // multihash 0x1220. 12: code sha2-256. 20: length (32 bytes)
        ...
        async prepareBlock(block) {
          ...
          if (block.transactions) {
            for (let index = 0; index < block.transactions.length; index++) {
              ...
              (_a = tx.signatures) === null || _a === void 0
                ? void 0
                : _a.forEach((sig) => {
          ...
          block.header = {
            ...
            transaction_merkle_root: (0, utils_1.encodeBase64url)(
              new Uint8Array([
                // multihash sha256: 18, 32
                18,
                32,
    ...
    /***/ 8593: /***/ function (
      ...
      exports.tokenAbi =
        exports.btypeEncode =
        exports.btypeDecode =
        exports.btypeEncodeValue =
        exports.btypeDecodeValue =
        exports.parseUnits =
        exports.formatUnits =
        exports.isChecksumWif =
        exports.isChecksumAddress =
        exports.isChecksum =
        exports.bitcoinAddress =
        exports.bitcoinDecode =
        exports.bitcoinEncode =
        exports.calculateMerkleRoot =
      ...
      function calculateMerkleRoot(hashes) {
        if (!hashes.length) return (0, sha256_1.sha256)(new Uint8Array());
        while (hashes.length > 1) {
          for (let i = 0; i < hashes.length; i += 2) {
            if (i + 1 < hashes.length) {
              const leftHash = hashes[i];
              const rightHash = hashes[i + 1];
              const sumHash = (0, sha256_1.sha256)(
                new Uint8Array([...leftHash, ...rightHash])
              );
    ...
    /***/ 9104: /***/ function (module, exports, __webpack_require__) {
      var __WEBPACK_AMD_DEFINE_FACTORY__,
        __WEBPACK_AMD_DEFINE_ARRAY__,
        __WEBPACK_AMD_DEFINE_RESULT__; /*eslint-disable block-scoped-var, id-length, no-control-regex, no-magic-numbers, no-prototype-builtins, no-redeclare, no-shadow, no-var, sort-vars*/
      (function (global, factory) {
        /* global define, require, module */

        /* AMD */ if (true)
          !((__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(2100)]),
        ...
        $root.koinos = (function () {
          ...
          koinos.protocol = (function () {
            ...
            protocol.event_data = (function () {
               ...
               * @returns {$protobuf.Writer} Writer
               */
              event_data.encodeDelimited = function encodeDelimited(
                message,
                writer
              ) {
              ...
              event_data.verify = function verify(message) {
                ...
                if (
                  message.sequence != null &&
                  message.hasOwnProperty("sequence")
                )
                ...
                if (
                  message.impacted != null &&
                  message.hasOwnProperty("impacted")
                ) {
            ...
            protocol.contract_call_bundle = (function () {
              ...
              contract_call_bundle.fromObject = function fromObject(object) {
                if (
                  object instanceof $root.koinos.protocol.contract_call_bundle
                )
                  return object;
               ...
               * @returns {Object.<string,*>} Plain object
               */
              contract_call_bundle.toObject = function toObject(
                message,
                options
              ) {
                if (!options) options = {};
               ...
               * @returns {string} The default type url
               */
              contract_call_bundle.getTypeUrl = function getTypeUrl(
                typeUrlPrefix
              ) {
                if (typeUrlPrefix === undefined) {
                  typeUrlPrefix = "type.googleapis.com";
            ...
            protocol.system_call_target = (function () {
               ...
               * @throws {$protobuf.util.ProtocolError} If required fields are missing
               */
              system_call_target.decodeDelimited = function decodeDelimited(
                reader
              ) {
                if (!(reader instanceof $Reader)) reader = new $Reader(reader);
                return this.decode(reader, reader.uint32());
              ...
              system_call_target.verify = function verify(message) {
                ...
                if (
                  message.thunk_id != null &&
                  message.hasOwnProperty("thunk_id")
                ) {
                  properties.target = 1;
                ...
                if (
                  ...
                  {
                    var error =
                      $root.koinos.protocol.contract_call_bundle.verify(
                        message.system_call_bundle
                      );
               ...
               * @returns {Object.<string,*>} Plain object
               */
              system_call_target.toObject = function toObject(
                message,
                options
              ) {
                if (!options) options = {};
               ...
               * @returns {string} The default type url
               */
              system_call_target.getTypeUrl = function getTypeUrl(
                typeUrlPrefix
              ) {
                if (typeUrlPrefix === undefined) {
                  typeUrlPrefix = "type.googleapis.com";
            ...
            protocol.upload_contract_operation = (function () {
               ...
               * @returns {$protobuf.Writer} Writer
               */
              upload_contract_operation.encode = function encode(
                message,
                writer
              ) {
                if (!writer) writer = $Writer.create();
                ...
                if (
                  ...
                  writer
                    .uint32(/* id 2, wireType 2 =*/ 18)
                    .bytes(message.bytecode);
                if (
                  message.abi != null &&
                  Object.hasOwnProperty.call(message, "abi")
                )
               ...
               * @returns {$protobuf.Writer} Writer
               */
              upload_contract_operation.encodeDelimited =
               ...
               * @throws {$protobuf.util.ProtocolError} If required fields are missing
               */
              upload_contract_operation.decode = function decode(
                reader,
                length
              ) {
                if (!(reader instanceof $Reader))
                ...
                var end =
                    length === undefined ? reader.len : reader.pos + length,
                  message =
                    new $root.koinos.protocol.upload_contract_operation();
                while (reader.pos < end) {
                  var tag = reader.uint32();
                  switch (tag >>> 3) {
                    case 1: {
               ...
               * @throws {$protobuf.util.ProtocolError} If required fields are missing
               */
              upload_contract_operation.decodeDelimited =
              ...
              upload_contract_operation.verify = function verify(message) {
                ...
                if (
                  message.bytecode != null &&
                  message.hasOwnProperty("bytecode")
                )
                  if (
                    !(
                      (message.bytecode &&
                ...
                if (
                  ...
                  if (
                    typeof message.authorizes_transaction_application !==
                    "boolean"
                  )
               ...
               * @returns {koinos.protocol.upload_contract_operation} upload_contract_operation
               */
              upload_contract_operation.fromObject = function fromObject(
                object
              ) {
                if (
                  object instanceof
              ...
              upload_contract_operation.toObject = function toObject(
                ...
                if (
                  ...
                  object.authorizes_call_contract =
                    message.authorizes_call_contract;
                if (
                  message.authorizes_transaction_application != null &&
                  message.hasOwnProperty("authorizes_transaction_application")
                )
              ...
              upload_contract_operation.getTypeUrl = function getTypeUrl(
                ...
                return (
                  typeUrlPrefix + "/koinos.protocol.upload_contract_operation"
                );
              };
            ...
            protocol.call_contract_operation = (function () {
               ...
               * @returns {$protobuf.Writer} Writer
               */
              call_contract_operation.encode = function encode(
                message,
                writer
              ) {
                if (!writer) writer = $Writer.create();
               ...
               * @returns {$protobuf.Writer} Writer
               */
              call_contract_operation.encodeDelimited =
                function encodeDelimited(message, writer) {
                  return this.encode(message, writer).ldelim();
                };
               ...
               * @throws {$protobuf.util.ProtocolError} If required fields are missing
               */
              call_contract_operation.decodeDelimited =
                function decodeDelimited(reader) {
                  if (!(reader instanceof $Reader))
                    reader = new $Reader(reader);
                  return this.decode(reader, reader.uint32());
              ...
              call_contract_operation.verify = function verify(message) {
                ...
                if (message.args != null && message.hasOwnProperty("args"))
                  if (
                    !(
                      (message.args &&
                        typeof message.args.length === "number") ||
              ...
              call_contract_operation.fromObject = function fromObject(object) {
                if (
                  ...
                  return object;
                var message =
                  new $root.koinos.protocol.call_contract_operation();
                if (object.contract_id != null)
                  if (typeof object.contract_id === "string")
                    $util.base64.decode(
                      object.contract_id,
               ...
               * @returns {Object.<string,*>} Plain object
               */
              call_contract_operation.toObject = function toObject(
                message,
                options
              ) {
                if (!options) options = {};
              ...
              call_contract_operation.getTypeUrl = function getTypeUrl(
                ...
                return (
                  typeUrlPrefix + "/koinos.protocol.call_contract_operation"
                );
              };
            ...
            protocol.set_system_call_operation = (function () {
               ...
               * @returns {$protobuf.Writer} Writer
               */
              set_system_call_operation.encode = function encode(
                message,
                writer
              ) {
                if (!writer) writer = $Writer.create();
               ...
               * @returns {$protobuf.Writer} Writer
               */
              set_system_call_operation.encodeDelimited =
                function encodeDelimited(message, writer) {
                  return this.encode(message, writer).ldelim();
                };
               ...
               * @throws {$protobuf.util.ProtocolError} If required fields are missing
               */
              set_system_call_operation.decode = function decode(
                reader,
                length
              ) {
                if (!(reader instanceof $Reader))
                ...
                var end =
                    length === undefined ? reader.len : reader.pos + length,
                  message =
                    new $root.koinos.protocol.set_system_call_operation();
                while (reader.pos < end) {
                  var tag = reader.uint32();
                  switch (tag >>> 3) {
                    case 1: {
               ...
               * @throws {$protobuf.util.ProtocolError} If required fields are missing
               */
              set_system_call_operation.decodeDelimited =
                function decodeDelimited(reader) {
                  if (!(reader instanceof $Reader))
                    reader = new $Reader(reader);
                  return this.decode(reader, reader.uint32());
              ...
              set_system_call_operation.verify = function verify(message) {
                ...
                if (
                  message.call_id != null &&
                  message.hasOwnProperty("call_id")
                )
               ...
               * @returns {koinos.protocol.set_system_call_operation} set_system_call_operation
               */
              set_system_call_operation.fromObject = function fromObject(
                object
              ) {
                if (
                  object instanceof
              ...
              set_system_call_operation.toObject = function toObject(
                ...
                if (message.target != null && message.hasOwnProperty("target"))
                  object.target =
                    $root.koinos.protocol.system_call_target.toObject(
                      message.target,
                      options
                    );
                return object;
              ...
              set_system_call_operation.getTypeUrl = function getTypeUrl(
                ...
                return (
                  typeUrlPrefix + "/koinos.protocol.set_system_call_operation"
                );
              };
            ...
            protocol.set_system_contract_operation = (function () {
               ...
               * @instance
               */
              set_system_contract_operation.prototype.contract_id =
                $util.newBuffer([]);
               ...
               */
              set_system_contract_operation.create = function create(
                properties
              ) {
               ...
               * @throws {$protobuf.util.ProtocolError} If required fields are missing
               */
              set_system_contract_operation.decode = function decode(
                reader,
                length
              ) {
                if (!(reader instanceof $Reader))
               ...
               * @returns {koinos.protocol.set_system_contract_operation} set_system_contract_operation
               */
              set_system_contract_operation.fromObject = function fromObject(
                object
              ) {
                if (
                  object instanceof
               ...
               * @returns {Object.<string,*>} JSON object
               */
              set_system_contract_operation.prototype.toJSON =
                function toJSON() {
                  return this.constructor.toObject(
                    this,
                    $protobuf.util.toJSONOptions
            ...
            protocol.operation = (function () {
               ...
               * @returns {$protobuf.Writer} Writer
               */
              operation.encodeDelimited = function encodeDelimited(
                message,
                writer
              ) {
                return this.encode(message, writer).ldelim();
              ...
              operation.verify = function verify(message) {
                ...
                if (
                  ...
                  {
                    var error =
                      $root.koinos.protocol.call_contract_operation.verify(
                        message.call_contract
                      );
            ...
            protocol.transaction_header = (function () {
               ...
               * @instance
               */
              transaction_header.prototype.operation_merkle_root =
                $util.newBuffer([]);
               ...
               * @throws {$protobuf.util.ProtocolError} If required fields are missing
               */
              transaction_header.decodeDelimited = function decodeDelimited(
                reader
              ) {
                if (!(reader instanceof $Reader)) reader = new $Reader(reader);
                return this.decode(reader, reader.uint32());
              ...
              transaction_header.verify = function verify(message) {
                ...
                if (
                  message.chain_id != null &&
                  message.hasOwnProperty("chain_id")
                )
                  if (
                    !(
                      (message.chain_id &&
                ...
                if (
                  message.rc_limit != null &&
                  message.hasOwnProperty("rc_limit")
                )
                  if (
                ...
                if (message.nonce != null && message.hasOwnProperty("nonce"))
                  if (
                    !(
                      (message.nonce &&
                        typeof message.nonce.length === "number") ||
                  ...
                  if (
                    !(
                      (message.operation_merkle_root &&
                        typeof message.operation_merkle_root.length ===
                          "number") ||
                ...
                if (message.payer != null && message.hasOwnProperty("payer"))
                  if (
                    !(
                      (message.payer &&
                        typeof message.payer.length === "number") ||
                ...
                if (message.payee != null && message.hasOwnProperty("payee"))
                  if (
                    !(
                      (message.payee &&
                        typeof message.payee.length === "number") ||
               ...
               * @returns {Object.<string,*>} Plain object
               */
              transaction_header.toObject = function toObject(
                message,
                options
              ) {
                if (!options) options = {};
               ...
               * @returns {string} The default type url
               */
              transaction_header.getTypeUrl = function getTypeUrl(
                typeUrlPrefix
              ) {
                if (typeUrlPrefix === undefined) {
                  typeUrlPrefix = "type.googleapis.com";
            ...
            protocol.transaction = (function () {
              ...
              transaction.fromObject = function fromObject(object) {
                ...
                if (object.id != null)
                  if (typeof object.id === "string")
                    $util.base64.decode(
                      object.id,
                      (message.id = $util.newBuffer(
                        $util.base64.length(object.id)
                      )),
                      0
                    );
              ...
              transaction.toObject = function toObject(message, options) {
                ...
                if (message.header != null && message.hasOwnProperty("header"))
                  object.header =
                    $root.koinos.protocol.transaction_header.toObject(
                      message.header,
                      options
                    );
                if (message.operations && message.operations.length) {
                  ...
                  for (var j = 0; j < message.operations.length; ++j)
                    object.operations[j] =
                      $root.koinos.protocol.operation.toObject(
            ...
            protocol.transaction_receipt = (function () {
               ...
               * @throws {$protobuf.util.ProtocolError} If required fields are missing
               */
              transaction_receipt.decodeDelimited = function decodeDelimited(
                reader
              ) {
                if (!(reader instanceof $Reader)) reader = new $Reader(reader);
                return this.decode(reader, reader.uint32());
              ...
              transaction_receipt.verify = function verify(message) {
                ...
                if (
                  message.rc_used != null &&
                  message.hasOwnProperty("rc_used")
                )
                  if (
              ...
              transaction_receipt.fromObject = function fromObject(object) {
                ...
                if (object.events) {
                  ...
                  for (var i = 0; i < object.events.length; ++i) {
                    ...
                    message.events[i] =
                      $root.koinos.protocol.event_data.fromObject(
                        object.events[i]
                      );
                  }
                }
               ...
               * @returns {Object.<string,*>} Plain object
               */
              transaction_receipt.toObject = function toObject(
                message,
                options
              ) {
                if (!options) options = {};
                ...
                if (message.events && message.events.length) {
                  ...
                  for (var j = 0; j < message.events.length; ++j)
                    object.events[j] =
                      $root.koinos.protocol.event_data.toObject(
                        message.events[j],
                        options
                      );
                }
               ...
               * @returns {string} The default type url
               */
              transaction_receipt.getTypeUrl = function getTypeUrl(
                typeUrlPrefix
              ) {
                if (typeUrlPrefix === undefined) {
                  typeUrlPrefix = "type.googleapis.com";
            ...
            protocol.block_header = (function () {
               ...
               * @instance
               */
              block_header.prototype.previous_state_merkle_root =
                $util.newBuffer([]);
              ...
              block_header.verify = function verify(message) {
                ...
                if (
                  message.previous != null &&
                  message.hasOwnProperty("previous")
                )
                  if (
                    !(
                      (message.previous &&
                  ...
                  if (
                    !(
                      (message.transaction_merkle_root &&
                        typeof message.transaction_merkle_root.length ===
                          "number") ||
                    ...
                    if (
                      !(
                        (message.approved_proposals[i] &&
                          typeof message.approved_proposals[i].length ===
                            "number") ||
            ...
            protocol.block = (function () {
               ...
               * @returns {$protobuf.Writer} Writer
               */
              block.encodeDelimited = function encodeDelimited(
                message,
                writer
              ) {
                return this.encode(message, writer).ldelim();
              ...
              block.decode = function decode(reader, length) {
                ...
                while (reader.pos < end) {
                  ...
                  switch (tag >>> 3) {
                    ...
                    case 2: {
                      message.header =
                        $root.koinos.protocol.block_header.decode(
                          reader,
                          reader.uint32()
                        );
                      break;
                    ...
                    case 3: {
                      if (
                        !(message.transactions && message.transactions.length)
                      )
              ...
              block.fromObject = function fromObject(object) {
                ...
                if (object.header != null) {
                  ...
                  message.header =
                    $root.koinos.protocol.block_header.fromObject(
                      object.header
                    );
                }
            ...
            protocol.block_receipt = (function () {
              ...
              block_receipt.verify = function verify(message) {
                ...
                if (
                  ...
                  for (
                    var i = 0;
                    i < message.transaction_receipts.length;
                    ++i
                  ) {
                    var error =
                      $root.koinos.protocol.transaction_receipt.verify(
                        message.transaction_receipts[i]
                      );
          ...
          koinos.chain = (function () {
            ...
            chain.value_type = (function () {
               ...
               * @returns {$protobuf.Writer} Writer
               */
              value_type.encodeDelimited = function encodeDelimited(
                message,
                writer
              ) {
                return this.encode(message, writer).ldelim();
              ...
              value_type.fromObject = function fromObject(object) {
                ...
                if (object.sfixed64_value != null)
                  if ($util.Long)
                    ...
                    message.sfixed64_value = parseInt(
                      object.sfixed64_value,
                      10
                    );
            ...
            chain.enum_type = (function () {
               ...
               * @returns {$protobuf.Writer} Writer
               */
              enum_type.encodeDelimited = function encodeDelimited(
                message,
                writer
              ) {
                return this.encode(message, writer).ldelim();
            ...
            chain.list_type = (function () {
               ...
               * @returns {$protobuf.Writer} Writer
               */
              list_type.encodeDelimited = function encodeDelimited(
                message,
                writer
              ) {
                return this.encode(message, writer).ldelim();
                  ...
                  switch (tag >>> 3) {
                    case 1: {
                      if (!(message.values && message.values.length))
                        message.values = [];
                      message.values.push(
                        $root.koinos.chain.value_type.decode(
                          reader,
                          reader.uint32()
                        )
                      );
              ...
              list_type.fromObject = function fromObject(object) {
                ...
                if (object.values) {
                  ...
                  for (var i = 0; i < object.values.length; ++i) {
                    ...
                    message.values[i] =
                      $root.koinos.chain.value_type.fromObject(
                        object.values[i]
                      );
                  }
                }
          ...
          google.protobuf = (function () {
            ...
            protobuf.Any = (function () {
              ...
              Any.verify = function verify(message) {
                ...
                if (
                  message.type_url != null &&
                  message.hasOwnProperty("type_url")
                )
